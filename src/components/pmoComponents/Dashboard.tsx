...
import SalesAnalyticsAlternative from 'pages/Dashboard-saas/sales-analyticsBudgetInDashboard';
...
const Overview = (): ReactElement => {
  ...
  const [overallStatusState, setOverallStatus] = useState<string[]>([
    'green',
    'yellow',
    'red',
  ]);
  const [kpiStates, setKpiStates] = useState<string[]>([
    'Behind ',
    'On Track ',
    'Achieved ',
  ]);
  ...
  const getOverview = (overViewProps: OverviewType): ReactElement => {
    ...
    const overallStatus = colorCounters.findIndex(
      item => item === getMax(colorCounters),
    );
  ...
  const getBudgetPieChart = () => {
    ...
    return (
      <SalesAnalyticsAlternative
        title={
          'Budget: Approved & Spent ' +
          formatBudgetDate(overviewData.budgetDate)
        }
        green={totalApproved}
        yellow={totalSpent}
